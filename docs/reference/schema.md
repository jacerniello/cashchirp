...
| `sp500_member_months` | table | the **monthly member panel** powering the counterfactual *index lab* (`/sp500/lab`): one row per `(month, permaticker)` the name was an S&P 500 member, carrying its **total return that month** (`ret`, from `sep.closeadj` — split- *and* dividend-adjusted) and its **begin-of-month market cap** (`weight_cap` = prior month-end `daily.marketcap`, the cap-weight basis for that month's return — the one-month lag baked in so the read side is a pure group-by), plus `ticker`/`name`/`sector`. The index is chain-linked at read time (per month: renormalise the surviving members' weights, `Σ wᵢ·retᵢ`, compound), so **removing any companies/sectors is just a filter + renormalise** — identical machinery for baseline and every scenario, so methodology bias cancels in the comparison; equal-weight is offered as the breadth contrast. **Faithful reconstruction, not the official index:** full-cap (no float) + quarterly membership forward-filled to months; validated vs SPY total return (0.96 monthly-return correlation, CAGR within ~0.7%/yr over 28y). ~164k rows. Built by `python -m core.scripts.build.build_sp500_member_months`; auto-refreshed after an SP500 load and by the orchestrator. Rebuild from `sp500`+`sep`+`daily`+`tickers` — never hand-edit. |
